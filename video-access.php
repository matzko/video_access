...
class Video_Access_Control
{
	...
	public function filter_user_has_cap( $user_caps = array(), $caps = null, $args = null )
	{
		if ( in_array( Video_Access_Model::VIEW_CAP, $caps ) ) {
			/**
			 * $args should be like
			 * array( 'view_video', [current user id id], [video id] )
			 */
			if ( $this->model->user_can_view_video( $args[1], $args[2] ) ) {
				$user_caps[Video_Access_Model::VIEW_CAP] = true;
			}
		}
		return $user_caps;
	}
...
error_log( 'trying to write image as ' . $full_file_name . ' from ' . $tmp_path );
	...
	public function user_can_view_video( $user_id = 0, $video_id = 0 )
	{
		$user_id = (int) $user_id;
		$video_id = (int) $video_id;
